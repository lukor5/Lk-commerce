...
export default createStore({
  ...
  actions: {
    ...
    async fetchInitialProductData({ commit }) {
      try {
        const response = await axios.get('http://13.50.242.240/shop/products');
    ...
    async fetchBasket({ commit, state }, basketId) {
      ...
      let url = 'http://13.50.242.240/shop/basket/';
      if (basketId) {
        url += basketId;
      }
      axios.get(url, {
    ...
    async fetchNotifications({ commit, state }) {
      if (state.hasInitialized) return;
      const url = 'http://13.50.242.240/shop/notifications';
      try {
    ...
    async fetchOrders({ commit, state }, user_id) {
      if (state.hasInitialized) return;
      let url = 'http://13.50.242.240/shop/user-orders';
    ...
    async fetchVouchers({ commit, state }, user_id) {
      if (state.hasInitialized) return;
      let url = 'http://13.50.242.240/shop/user-vouchers';
      axios.get(`${url}/${user_id}`).then(response => {
